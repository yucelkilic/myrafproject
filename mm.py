	...
	if self.ui.lineEdit_2.text() != "" and self.ui.lineEdit_3.text() != "" and self.ui.lineEdit_4.text() != "":
		...
		if ch ==0:
			...
			if self.ui.checkBoxAlign.checkState() == QtCore.Qt.Checked:
				...
				os.system("echo " + str(c2x) + " " + str(c2y) + " >> ./tmp/coo")
	...
	if ix == "" or ye == "":
		...
		if a:
			if self.ui.radioButton.isChecked():
				...
				cor="x:" + str(round(float(event.pos().x())*float(ix)/float(dispX),2)) + " - y:" + str(round((436-float(event.pos().y()))*float(ye)/float(dispY),2))
				self.ui.lineEdit_4.setText(QtGui.QApplication.translate("Form", str(cor), None, QtGui.QApplication.UnicodeUTF8))
