...
def partitions(workload):
    ...
    while len(partition) != nCPU and len(workload) != 0:
        ...
        size = int(math.ceil(float(len(workload)) / float(n)))
...
def detect_candidates(CMO,
    ...
    for catalog in catalogs:
        ...
        for i in range(len(catalogF.x)):
            ...
            if len(masterF[math.pow((masterF.x - catalogF.x[i]), 2) +
                           math.pow((masterF.y - catalogF.y[i]), 2) <=
                           math.pow(TRAVEL_MIN, 2)]) < 2:
...
def all_candidates(catdir, outdir):
    ...
    #For sake of using python2x and python3x both
    #
