...
I pimped this a bit to suit my needs :
    ...
    - Automatic recognition of saturated stars, including their full
        ...
        Indeed saturated stars tend to get even uglier when you try to
        clean them. Plus they keep L.A.Cosmic iterations going on forever.
        This feature is mainly for pretty-image production. It is optional,
        requires one more parameter (a CCD saturation level in ADU), and uses
        some nicely robust morphology operations and object extraction.

    - Scipy image analysis allows to "label" the actual cosmic ray hits
        (i.e. group the pixels into local islands). A bit special, but I use
        this in the scope of visualizing a PSF construction.
...
Ideas for future improvements :
        ...
        - Add something reliable to detect negative glitches
                (dust on CCD or small traps)
...
class cosmicsimage:
    ...
    def __init__(self, rawarray, pssl=0.0, gain=2.2, readnoise=10.0,
        sigclip=5.0, sigfrac=0.3, objlim=5.0, satlevel=50000.0, verbose=False):
        """
        ...
        objlim : increase this if normal stars are detected as cosmics.
            Default is 5.0, a good value for earth-bound images.
            Constructor of the cosmic class, takes a 2D numpy array
            of your image as main argument.
        sigclip : laplacian-to-noise limit for cosmic ray detection
        objlim : minimum contrast between laplacian
            image and fine structure image.
            Use 5.0 if your image is undersampled, HST, ...
        satlevel : if we find agglomerations of pixels above this level,
            we consider it to be a saturated star and
            do not try to correct and pixels around it.
            A negative satlevel skips this feature.

        pssl is the previously subtracted sky level !
        # gain (electrons/ADU)	(0=unknown)
        real   gain    = 1.8
        # read noise (electrons) (0=unknown)
        real   readn   = 6.5
        # section to use for automatic computation of gain
        ##gain0  string statsec = "*,*"
        # sky level that has been subtracted (ADU)
        real   skyval  = 0.
        # detection limit for cosmic rays (sigma)
        real   sigclip = 3.0
        # fractional detection limit for neighbouring pixels
        real   sigfrac = 0.5
        # contrast limit between CR and underlying object
        real   objlim  = 3.0
        ...
        def __str__(self):
            """
            Gives a summary of the current state, including the
                number of cosmic pixels in the mask etc.
            """
            stringlist = [
                "Input array : (%i, %i), %s" % (self.rawarray.shape[0],
                    self.rawarray.shape[1], self.rawarray.dtype.name),
            ...
            if self.satstars is not None:
                stringlist.append("Saturated star mask : %i pixels" % np.sum(
                    self.satstars))
        ...
        def labelmask(self, verbose=None):
            ...
            This is made on purpose for visualizations a la f2n.drawstarslist,
            ...
            # We morphologicaly dilate the mask to generously connect
            #    "sparse" cosmics :
            #dilstruct = np.ones((5,5))
            dilmask = ndimage.morphology.binary_dilation(self.mask,
                structure=dilstruct, iterations=1, mask=None, output=None,
            ...
            # Now we have a huge list of couples of numpy slice objects giving
            #    a frame around each object
            # For plotting purposes, we want to transform this into the
            ...
            # We also want to know how many pixels
            #    where affected by each cosmic ray.
            # Why ? Dunno... it's fun and available in scipy :-)
            sizes = ndimage.measurements.sum(self.mask.ravel(),
                labels.ravel(), np.arange(1, n + 1, 1))
            retdictlist = [{"name":"%i" % size, "x":center[0],
                "y":center[1]} for (size, center) in zip(sizes, centers)]
        ...
        def getdilatedmask(self, size=3):
            ...
            if size == 3:
                dilmask = ndimage.morphology.binary_dilation(self.mask,
                    structure=growkernel, iterations=1, mask=None, output=None,
                    border_value=0, origin=0, brute_force=False)
            elif size == 5:
                dilmask = ndimage.morphology.binary_dilation(self.mask,
                    structure=dilstruct, iterations=1, mask=None, output=None,
        ...
        def clean(self, mask=None, verbose=None):
            ...
            masked 5x5 median value. This mimics what is done in L.A.Cosmic,
            but it's a bit harder to do in python, as there is no readymade
            masked median. So for now we do a loop...
            Saturated stars, if calculated, are also masked : they are not
            "cleaned", but their pixels are not used for the interpolation.

            We will directly change self.cleanimage. Instead of using the self.
            mask, you can supply your own mask as argument. This might be useful
            to apply this cleaning function iteratively. But for the true
            L.A.Cosmic, we don't use this, i.e. we use the full mask at each
            iteration.
            """
            if verbose is None:
                verbose = self.verbose
            ...
            # So... mask is a 2D array containing False and True,
            #    where True means "here is a cosmic"
            ...
            # Now we want to have a 2 pixel frame of Inf padding
            #    around our image.
            ...
            # The medians will be evaluated in this padarray,
            #    skipping the np.Inf.
            # Now in this copy called padarray, we also put the saturated
            #    stars to np.Inf, if available :
            ...
            for cosmicpos in cosmicindices:
                ...
                #print cutout
                # Now we have our 25 pixels, some of them are np.Inf,
                #    and we want to take the median
                ...
                if np.alen(goodcutout) >= 25:
                    ...
                    # i.e. no good pixels : Shit, a huge cosmic, we will
                    #    have to improvise ...
                    print("OH NO, I HAVE A HUUUUUUUGE COSMIC !!!!!")
                    replacementvalue = self.guessbackgroundlevel()
                    # We update the cleanarray,
                    # but measure the medians in the padarray,
                    #    so to not mix things up...
                ...
                # ok so outmask = 1 are the cosmics
                ...
                if (i>1) imdel(output)
                imcalc(oldoutput//","//outmask//",
                    "//med5,output,"(1.-im2)*im1+im3",verb-)

                # =

                merging to full mask
                # So this is 1.0, but cosmics are very negative
                inputmask = 1.0 - 10000.0 * finalsel
                # orig image, with very negative cosmics
                inputmask = oldoutput * inputmask
        ...
        def findsatstars(self, verbose=None):
            """
            Uses the satlevel to find saturated stars (not cosmics !),
            and puts the result as a mask in self.satstars. This can then be
            ...
            if verbose:
                ...
                # We build a smoothed version of the image to look for
                #    large stars and their support :
                m5 = ndimage.filters.median_filter(self.rawarray,
                    size=5, mode='mirror')
                ...
                # We dilate the satpixels alone, to ensure connectivity in
                #    glitchy regions and to add a safety margin around them.
        ...
        def guessbackgroundlevel(self):
            """
            Estimates the background level. This could be used to
                fill pixels in large cosmics.
            """
        ...
        def lacosmiciteration(self, verbose=None):
            ...
            It operates on self.cleanarray, and afterwards updates self.
                mask by adding the newly detected
            cosmics to the existing self.mask. Cleaning is not made
            ...
            If findsatstars() was called, we exclude these regions from the
                search.
            """
            if verbose is None:
                verbose = self.verbose
            ...
            # We subsample, convolve, clip negative
            #    values, and rebin to original size
            subsam = subsample(self.cleanarray)
            conved = signal.convolve2d(subsam, laplkernel,
                mode="same", boundary="symm")
            cliped = conved.clip(min=0.0)
            #cliped = np.abs(conved) # unfortunately this does not work
            #    to find holes as well ...
            ...
            m5 = ndimage.filters.median_filter(self.cleanarray,
                size=5, mode='mirror')
            # We keep this m5, as I will use it later for the interpolation.
            # As we will take the sqrt
            ...
            f = m3 - m37
            # In the article that's it, but in lacosmic.cl f
            #    is divided by the noise...
            # Ok I understand why, it depends on if
            #    you use sp/f or L+/f as criterion.
            # There are some differences between the article
            #    and the iraf implementation.
            ...
            if self.satstars is not None:
                ...
                finalsel = np.logical_and(
                    np.logical_not(self.satstars), finalsel)
            ...
            if verbose:
                ...
                # we outsource this to the function clean(), as for some
                #    purposes the cleaning might not even be needed.
                ...
                return {"niter": nbfinal, "nnew": nbnew,
                    "itermask": finalsel, "newmask": newmask}

        def findholes(self, verbose=True):
            """
            Detects "negative cosmics" in the cleanarray and adds
                them to the mask.
        ...
        def run(self, maxiter=4, verbose=False):
            ...
            Full artillery :-)
                ...
                - Run maxiter L.A.Cosmic iterations (stops if no more
                    cosmics are found)
            ...
            for i in range(1, maxiter + 1):
                ...
                #self.clean(mask = iterres["mask"]) # No, we want clean to
                #    operate on really clean pixels only !
...
# 	"""
# 	Applies the full artillery of the function
#            fullarray() directly on FITS files.
# 	"""
# 	pass
...
def tofits(outfilename, pixelarray, hdr=None, verbose=True):
    ...
    If you specify a header (pyfits format, as returned by fromfits())
        it will be used for the image.
    You can give me boolean numpy arrays, I will convert them
        into 8 bit integers.
    """
    pixelarrayshape = pixelarray.shape
    if verbose:
    ...
    # then a minimal header will be created
    if hdr is None:
        hdu = pyfits.PrimaryHDU(pixelarray.transpose())
        # this if else is probably not needed but anyway ...
    else:
...
def subsample(a):
    """
    Returns a 2x2-subsampled version of array a
        (no interpolation, just cutting pixels in 4).
    The version below is directly from the scipy cookbook on rebinning :
    U{http://www.scipy.org/Cookbook/Rebinning}
    There is ndimage.zoom(cutout.array, 2, order=0,
        prefilter=False), but it makes funny borders.
    """
    """
    # Ouuwww this is slow ...
    ...
    slices = [slice(
        0, old, float(old) / new) for old, new in zip(a.shape, newshape)]
...
def rebin(a, newshape):
    ...
    #print factor
    evList = ['a.reshape('] + ['newshape[%d],factor[%d],' % (
        i, i) for i in range(lenShape)] + [')'] + ['.sum(%d)' % (
        i + 1) for i in range(lenShape)] + ['/factor[%d]' % (
        i) for i in range(lenShape)]
